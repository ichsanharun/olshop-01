  ...
  <div class="modal-dialog modal-lg" width="100px" style="width:100% !important;margin-left:0.75%">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-warning" onclick="save_edit_list()">
...
<script type="text/javascript">
    ...
    function edit_list(noso){
      var param = $('#input_edit').val();
      //var uri3 = $('#no_so_pending').val();
      //window.location.href = siteurl+"salesorder/edit_list/"+noso+"?param="+param;
      tujuan = siteurl+'salesorder/edit_barang_so/'+noso+'?param='+param;
    ...
    function hapus_list(noso){
      var param = $('#input_edit').val();
      //var uri3 = $('#no_so_pending').val();
      //window.location.href = siteurl+"salesorder/edit_list/"+noso+"?param="+param;
      tujuan = siteurl+'salesorder/hapus_barang_so/'+noso+'?param='+param;
      swal({
        title: "Anda Yakin?",
      ...
      function(isConfirm){
        if (isConfirm) {
          $.ajax({
                  url: siteurl+'salesorder/hapus_barang_so/'+noso+'?param='+param,
                  data :{"NO_SO":noso,"ID":param},
                  dataType : "json",
                  type: 'POST',
                  success: function(result){
                      if(result.delete == '1'){
    ...
    $('#form-detail-so').on('submit', function(e){
      ...
      $.ajax({
        url: siteurl+"salesorder/saveitemso_edit",
        dataType : "json",
        type: 'POST',
        data: formdata,
        success: function(result){
    ...
    function save_edit_list(){
      //e.preventDefault();
      var formdata = $("#form-detail-so-pending").serialize();
      $.ajax({
        url: siteurl+"salesorder/save_edit_so",
        dataType : "json",
        type: 'POST',
        data: formdata,
        success: function(result){
    ...
    function saveheaderso_edit(){
        ...
        if($('#idcustomer').val() != ""){
        $.ajax({
            url: siteurl+"salesorder/saveheaderso_edit",
            dataType : "json",
            type: 'POST',
            data: formdata,
            success: function(result){
