      ...
      <select id="id_supplier" name="id_supplier" class="form-control input-sm" style="width: 25%;" tabindex="-1" required onchange="getsupplier()">
          ...
          <?php
            ...
            foreach(@$supplier as $kc=>$vc){
              ...
              ?>
          <option value="<?php echo $vc->id_supplier; ?>" <?php echo set_select('id_supplier', $vc->id_supplier, isset($data->nm_supplier) && $data->id_supplier == $vc->id_supplier) ?> <?php echo $selected?>>
...
<div class="text-right">
  <div class="box active">
    <div class="box-body">
      ...
      <button onclick="proses_po()" class="btn btn-primary" id="btn-proses-do" type="button"> Proses PO</button>
    </div>
  </div>
</div>
...
<script type="text/javascript">
  $(document).ready(function(){
      ...
      $('#btn-proses-do').prop('disabled', true);
  });
  ...
  function ceksup(id_supplier,no,id,set){
    ...
    var cek = $('#'+set);
    var cek_class = $('input.set_choose_po');
    //alert(cek.value);
    if (cek_class.is(":checked")) {
      $('#btn-proses-do').prop('disabled', false);
    }
    else{
      $('#btn-proses-do').prop('disabled', true);
    }
