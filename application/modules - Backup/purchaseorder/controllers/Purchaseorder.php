...
class Purchaseorder extends Admin_Controller {
    ...
    function saveheaderpo(){
        ...
        $pajak = $this->Purchaseorder_model->cek_data('ppn IS NOT NULL','parameter');
        ...
        for($i=0;$i < count($detail['nopr_topo']);$i++){
            ...
            $dataitem_po = array(
                ...
                'qty_acc' => $this->input->post('qty_supply')[$i],
            ...
            if($checkqty == $this->input->post('qty_pr')[$i]){
                ...
                $this->db->update('trans_pr_detail',array('proses_po'=>'PO','qty_po'=>$checkqty));//Detail PO sudah semua
                $this->db->where(array('no_pr' => $_POST['nopr_topo'][$i]))
                ->update('trans_pr_header',array('proses_po'=>'PO'));//Detail PO sudah semua
            }else{
                $this->db->where($keyclose_pr);
